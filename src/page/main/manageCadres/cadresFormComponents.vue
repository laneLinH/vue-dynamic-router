<template>
  <el-form  class="resum-form"
    ...
    <el-form-item label="所属组织部门"
      ...
      <el-input type="hidden" style="display: none" v-model="formData.orgName" ></el-input>
    </el-form-item>
    ...
    <el-form-item
      ...
      <el-radio-group
        ...
        <el-radio v-for="(che,ix) in partyInfo.options" style="margin-right: 11px;" :key="ix" :disabled="che.disabled" :label="che.label">{{che.label}}</el-radio>
      </el-radio-group>
    </el-form-item>
    ...
    <el-form-item    label="健康状况"
      ...
      <el-select type="text" filterable v-model="formData.healthCondition" >
        <el-option v-for="(item,key) in healthInfo.options" :value="item.value" :label="item.label" :key="key">
        </el-option>
      </el-select>
    ...
    <el-form-item v-if="healthInfo.isShowInstruction"   label="健康状况补充说明"
                  prop="healthInstruction"
                  :rules="rules.conditionInstuction">
      <el-input type="text" v-model="formData.healthInstruction"></el-input>
    </el-form-item>
...
<script>
    ...
    export default {
        ...
        data:()=>({
            ...
            healthInfo:{
                options:[ {value:'健康',label:'健康'},{value:'一般',label:'一般'},{value:'较差',label:'较差'},{value:'其它',label:'其它'}],
            ...
            partyInfo:{
                partyTypes:'中共党员',
                options:[{label:'中共党员',disabled:false},{label:'共青团员',disabled:false},{label:'群众',disabled:false},{label:'其它',disabled:false}],
                isShowDate:true,
            ...
            rules:{
                ...
                conditionInstuction:[{required: true, message: '请输入健康情况说明', trigger: 'change' }],
        ...
        watch:{
            ...
            'formData.healthCondition'(val){
                this.anycHealthy(val)
            }
        },
        computed:{
            ...mapState(['dynamicVx']),
        ...
        methods:{
            getcadreByorgNo(){
                this.$http.get(this.$api.cadreBase_curentinfo,{cadreId:this.formData.cadreId}).then((res)=>{
                    if(res.success){
                        ...
                        if(data.partyType==='中共党员'){
                            this.partyInfo.partyTypes='中共党员'
                            this.partyInfo.isShowDate=true
                        }else{
                            this.partyInfo.partyTypes=data.partyType
            ...
            anycHealthy(val){
                if(val==='其它') {
                    this.healthInfo.isShowInstruction = true
                }else{
                    this.healthInfo.isShowInstruction = false
                    this.formData.healthInstruction=null
                }
            },
            validPartyType(val) {
                this.formData.partyTypeList=val
                if(val&&val.includes('中共党员')) {
                    this.partyInfo.isShowDate = true
                }else{
                    this.formData.partyType=val
                    this.partyInfo.isShowDate = false
